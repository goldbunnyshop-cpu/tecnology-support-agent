...
COMANDOS_VALIDOS = ("listo", "demora", "presupuesto", "diagnostico", "password", "llamar", "cita", "pausa", "reanudar", "clabe", "pago", "nota", "orden", "estatus", "consultar", "marcar seguimiento", "detener seguimiento", "convertido")
...
TEXTO_MENU = (
    ...
    "*convertido:* [número], [nota_servicio] → Cliente atendido, detener seguimiento\n"
    "   _Ej: convertido: 5541576331, NS-001_"
)
...
async def procesar_comando_grupo(
    ...
    # ── convertido ──
        ...
        if len(partes) < 1:
            await _responder_grupo("⚠️ Formato: convertido: NÚMERO, NOTA_SERVICIO\n   _Ej: convertido: 5541576331, NS-001_")
            return True
        try:
            from agent.leads import marcar_como_convertido
            telefono = partes[0].strip()
            numero_nota = partes[1].strip() if len(partes) > 1 else ""

            # Normalizar teléfono
            telefono_norm = _normalizar_numero(telefono)

            resultado = await marcar_como_convertido(telefono_norm, numero_nota)
            if resultado:
                await _responder_grupo(f"✅ *CONVERTIDO* — {telefono_norm}\n📋 NS: {numero_nota}\n⏸️ Seguimiento detenido")
            else:
                await _responder_grupo(f"❌ No se encontró cliente con '{telefono}'")
        except Exception as e:
            await _responder_grupo(f"❌ Error: {e}")
