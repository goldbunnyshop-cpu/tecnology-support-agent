# AgentKit — Sistema de Instrucciones para Codex

> Este archivo es el CEREBRO de AgentKit. Codex lo lee automáticamente
...
| IA | Anthropic Codex API | Modelo: `Codex-sonnet-4-6` |
...
Codex genera esta estructura completa para cada usuario:
...
│   ├── brain.py           ← Conexión Codex API + system prompt desde prompts.yaml
...
Memory (agent/memory.py) — recupera historial de esa conversación
    ↓
Brain (agent/brain.py) — llama Codex API con: system prompt + historial + mensaje nuevo
    ↓
Codex API (Codex-sonnet-4-6) — genera respuesta inteligente
    ↓
...
Codex genera SOLO el proveedor que el usuario eligió (no los 3).
...
async def webhook_handler(request: Request):
    ...
    Procesa el mensaje, genera respuesta con Codex y la envía de vuelta.
    """
    try:
...
```python
# agent/brain.py — Cerebro del agente: conexión con Codex API
# Generado por AgentKit

"""
Lógica de IA del agente. Lee el system prompt de prompts.yaml
y genera respuestas usando la API de Anthropic Codex.
"""
...
async def generar_respuesta(mensaje: str, historial: list[dict]) -> str:
    """
    Genera una respuesta usando Codex API.
    ...
    Returns:
        La respuesta generada por Codex
    """
    ...
    try:
        ...
        logger.error(f"Error Codex API: {e}")
...
Codex genera las funciones según los casos de uso elegidos en la entrevista.
"""
...
# ════════════════════════════════════════════════════════════
# Codex: agrega aquí las funciones específicas según
...
Codex genera SOLO las variables del proveedor elegido (no las de los otros):
...
**Si hay archivos en `/knowledge`:** Codex debe leerlos (txt, pdf, csv, md, json, docx)
...
3. **IMPORTANTE: Antes de subir a GitHub, reemplazar el .gitignore.**
   ...
   Codex DEBE generar un nuevo `.gitignore` de producción:
...
5. **Resumen final:**
   ...
   - Cerebro con Codex AI (Codex-sonnet-4-6)
...
## 5. Reglas de comportamiento para Codex
